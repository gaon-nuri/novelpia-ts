function get_comment_in_writer() {
    _dev.log('get_comment_in_writer');
    $.ajax({
        url: "/proc/user",
        data: {
        ...
        success: function (response) {
            ...
            _dev.log(response.result);
            if (response.result != '') {
                g_is_writer = response.result.cnt;
            }
        },
        complete: function (userBlockCustom) {
            get_user_comment();
        }
    });
}
    ...
    if (g_mem_no == g_cate || g_mem_admin > 9) {
        ...
        if (g_member_view.view_comment == 1) {
            let _html = `
					...
					<div class="list-tit s_inv" style="justify-content: center; padding: 40px 0px; text-align: center;">
